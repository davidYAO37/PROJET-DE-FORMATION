...
interface Props {
    data: { _id: string; Modepaiement: string }[];
...
export default function ListeModePaiement({ data, onEdit, onDelete, onSelect, selectedId }: Props) {
    return (
        <Table bordered hover responsive className="mt-3 text-center align-middle">
            ...
            <tbody>
                {data.length > 0 ? (
                    ...
                    <tr>
                        <td colSpan={2}>Aucun mode de paiement enregistré.</td>
                    </tr>
                )}
            </tbody>
        </Table>
