...
type Props = {
    show: boolean;
    onHide: () => void;
    ParamLabo: ParamLabo | null;
    onSave: (param: ParamLabo) => void;
};

export default function ModifierParam({ show, onHide, ParamLabo, onSave }: Props) {
    ...
    // Remplir le formulaire quand on reçoit un paramètre
    useEffect(() => {
        if (ParamLabo) {
            setForm({
                NUM_PARAM: ParamLabo.NUM_PARAM || 0,
                ParamAbrege: ParamLabo.ParamAbrege || "",
                Param_designation: ParamLabo.Param_designation || "",
                PlageRefMinNe: ParamLabo.PlageRefMinNe || 0,
                PlageRefMaxNé: ParamLabo.PlageRefMaxNé || 0,
                UnitéParam: ParamLabo.UnitéParam || "",
                PlageMinMaxNé: ParamLabo.PlageMinMaxNé || "",
                PlageMinEnfant: ParamLabo.PlageMinEnfant || 0,
                PlageMaxEnfant: ParamLabo.PlageMaxEnfant || 0,
                PlageMinMaxEnfant: ParamLabo.PlageMinMaxEnfant || "",
                PLageMinFemme: ParamLabo.PLageMinFemme || 0,
                PlageMaxFemme: ParamLabo.PlageMaxFemme || 0,
                PlageMinMaxFemme: ParamLabo.PlageMinMaxFemme || "",
                PlageMinHomme: ParamLabo.PlageMinHomme || 0,
                PlageMaxHomme: ParamLabo.PlageMaxHomme || 0,
                PlageMinMaxHomme: ParamLabo.PlageMinMaxHomme || "",
                ValeurNormale: ParamLabo.ValeurNormale || "",
                ValeurMinNormale: ParamLabo.ValeurMinNormale || 0,
                ValeurMaxNormale: ParamLabo.ValeurMaxNormale || 0,
                TypeTexte: ParamLabo.TypeTexte || false,
                EstLibelle: ParamLabo.EstLibelle || false,
                // Ajouter les signes pour chaque section
                SigneNormale: ParamLabo.SigneNormale || "-",
                SigneNé: ParamLabo.SigneNé || "-",
                SigneEnfant: ParamLabo.SigneEnfant || "-",
                SigneFemme: ParamLabo.SigneFemme || "-",
                SigneHomme: ParamLabo.SigneHomme || "-",
            });
        }
    }, [ParamLabo]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/paramlabo/${ParamLabo._id}`, {
                method: "PUT",
            ...
            const data: ParamLabo = await res.json();
            onSave(data);
    ...
    return (
        <Modal show={show} onHide={onHide} size="xl">
            <Modal.Header closeButton>
                <Modal.Title>Modifier le Paramètre Laboratoire</Modal.Title>
            </Modal.Header>
