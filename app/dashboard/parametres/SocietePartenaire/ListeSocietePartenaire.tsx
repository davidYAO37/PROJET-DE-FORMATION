...
interface Props {
    data: SocietePartenaire[];
    onEdit: (item: SocietePartenaire) => void;
    onDelete: (id: string) => void;
    onSelect: (item: SocietePartenaire) => void;
    selectedId: string | null;
}

export default function ListeSocietePartenaire({ data, onEdit, onDelete, onSelect, selectedId }: Props) {
    return (
        <Table bordered hover responsive className="mt-3 text-center align-middle">
            ...
            <tbody>
                {data.length > 0 ? (
                    ...
                    <tr>
                        <td colSpan={2}>Aucune société partenaire enregistrée.</td>
                    </tr>
                )}
            </tbody>
        </Table>
