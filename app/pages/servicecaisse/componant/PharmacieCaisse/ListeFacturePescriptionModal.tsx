...
export default function ListeFactureModal({
    ...
    useEffect(() => {
        if (!show || !IDPRESCRIPTION) return;

        const fetchFactures = async () => {
            setLoading(true);
            try {
                const response = await fetch(`/api/facturesPrescriptionListe?IDPRESCRIPTION=${IDPRESCRIPTION}`);
            ...
            <Modal.Body className="p-0">
                <div className="table-responsive" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                    {loading ? (
                        ...
                        <div className="text-center my-4">
                            <p>Aucune facture trouvée pour cette prescription.</p>
                            <p className="text-muted small">ID Prescription: {IDPRESCRIPTION}</p>
                            <button
                                className="btn btn-sm btn-outline-secondary mt-2"
                                onClick={() => console.log('État actuel:', { factures, IDPRESCRIPTION })}
                            >
