...
// Composant principal
export default function ActionsButtonsCaisseManuelle({
  disabled = false,
  formData,
  lignes = [],
  ...
  // NB: le tableau manuel (ActesTableCaisseManuelle) utilise des noms de champs (Acte, QteP,
  // Coefficient, Prixunitaire, PrixTotal, PartAssurance, PartAssure, IDACTE, IDHOSPO...) qui ne
  // correspondent pas au schéma LignePrestation (prestation, qte, coefficientActe, prix,
  // prixTotal, partAssurance, partAssure, idActe, idHospitalisation, CodePrestation requis...).
  // On effectue donc la conversion ici avant l'appel à l'API.
  const mettreAJourLignes = async (examenId: string) => {
    const nomUtilisateur = localStorage.getItem("nom_utilisateur") || "Système";

    const promises = lignes.map(async (ligne) => {
      const isPaye = ligne.AFacturer === 'Payé';
      ...
      const lignePayload = {
        ...
        dateLignePrestation: ligne.DATE ? new Date(ligne.DATE) : new Date(),
        prestation: ligne.Acte || "",
        lettreCle: ligne.Lettre_Cle || "",
        idActe: ligne.IDACTE || undefined,
        idTypeActe: ligne.IDTYPE || undefined,

        // Montants saisis manuellement par l'utilisateur
        coefficientActe: ligne.Coefficient || 1,
        qte: ligne.QteP || 1,
        prix: ligne.Prixunitaire || 0,
        prixTotal: ligne.PrixTotal || 0,
        partAssurance: ligne.PartAssurance || 0,
        partAssure: ligne.PartAssure || 0,
        tauxAssurance: 0,
        reliquatCoefAssurance: ligne.TotalRelicatCoefAssur || 0,
        reliquatPatient: ligne.Reliquat || 0,
        totalCoefficient: ligne.CoefClinique || undefined,
        taxe: ligne.TAXE || 0,
        exclusionActe: ligne.Exclusion || "Accepter",
        ordonnancementAffichage: ligne.ordonnancementAffichage || 0,

        // Attribution médecin exécutant / anesthésiste / aide opératoire
        numMedecinExecutant: ligne.numMedecinExecutant || "",
        medecinExecutant: ligne.medecinExecutant || "",
        montantMedecinExecutant: ligne.Montant_MedExecutant || 0,
        IDAnesthesiste: ligne.IDAnesthesiste || "",
        StatutMedecinAnesthesiste: ligne.StatutMedecinAnesthesiste || "NON",
        IDmedecinAideOperatoire: ligne.IDmedecinAideOperatoire || "",
        StatutMedecinAideOperatoire: ligne.StatutMedecinAideOperatoire || "NON",
        MedecinAffiche: ligne.MedecinAffiche || "",

        statutPrescriptionMedecin: isPaye ? 3 : 1, // 3 = Payé, 1 = Non payé
        actePayeCaisse: isPaye ? "OUI" : "NON",
        datePaiementCaisse: isPaye ? new Date() : undefined,
        heurePaiement: isPaye ? (ligne.heurePaiement || new Date().toLocaleTimeString('fr-FR')) : undefined,
        payePar: isPaye ? nomUtilisateur : undefined,
      };

      const IDLignePrestation = ligne.IDLignePrestation;
      // Vérifier si c'est un ObjectId MongoDB valide (ligne déjà enregistrée en base)
      const isExistingLigne = IDLignePrestation && IDLignePrestation.length === 24 && /^[0-9a-fA-F]{24}$/.test(IDLignePrestation);

      // Pour les nouvelles lignes (sans ID en base), on fait un POST
      if (!isExistingLigne) {
