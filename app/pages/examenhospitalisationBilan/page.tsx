...
import PaiementInfoBilan from "./components/PaiementInfoBilan";
import ActionsButtonsBilan from "./components/ActionsButtonsBilan";
import CliniqueInfoBilan from "./components/CliniqueInfoBilan";
import TablePrestationsBilan from "./components/ActesTableBilan";

export default function HospitalisationPageBilan() {
    ...
    const [societesPartenaires, setSocietesPartenaires] = useState<{ _id: string; Designation: string }[]>([]);
    ...
    // Charger les types actes (uniquement EXAMEN BIOLOGIQUE)
    useEffect(() => {
        fetch("/api/typeacte")
            .then((res) => res.json())
            .then((data) => {
                if (Array.isArray(data)) {
                    setTypesActe(data.filter((t: any) => t.Designation === "EXAMEN BIOLOGIQUE"));
                }
            })
            .catch(() => setTypesActe([]));
    }, []);

    // Charger les sociétés partenaires
    useEffect(() => {
        fetch("/api/societePartenaire")
            .then((res) => res.json())
            .then((data) => {
                if (Array.isArray(data)) setSocietesPartenaires(data);
            })
            .catch(() => setSocietesPartenaires([]));
    }, []);
