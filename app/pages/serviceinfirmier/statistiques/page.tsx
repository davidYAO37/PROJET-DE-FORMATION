...
interface Totals {
  ...
  consultationsAvecConstantes: number;
  hospitalisations: number;
  hospitalisationsEnCours: number;
  soins: number;
  constantesHospit: number;
}

interface TypeSoinItem {
  label: string;
  value: number;
  color: string;
}
...
const kpiCards = (totals: Totals) => [
  { label: 'Consultations', value: totals.consultations, icon: 'bi-clipboard-plus', color: 'primary', detail: true },
  { label: 'Constantes clinique', value: totals.consultationsAvecConstantes, icon: 'bi-clipboard2-pulse', color: 'danger', detail: true },
  { label: 'Hospitalisations', value: totals.hospitalisations, icon: 'bi-hospital', color: 'warning', detail: false },
  { label: 'Hospitalisés en cours', value: totals.hospitalisationsEnCours, icon: 'bi-hospital-fill', color: 'info', detail: false },
  { label: 'Soins administrés', value: totals.soins, icon: 'bi-bandaid', color: 'success', detail: false },
  { label: 'Constantes hospitalisation', value: totals.constantesHospit, icon: 'bi-activity', color: 'secondary', detail: false }
];

export default function StatistiquesInfirmierPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await fetch(`/api/infirmier/statistiques?${params.toString()}`);
  ...
  const fetchDetails = async (title: string, params: URLSearchParams, message: string = '') => {
    try {
      ...
      const res = await fetch(`/api/consultation/liste?${params.toString()}`);
      if (!res.ok) throw new Error('Erreur lors du chargement des détails');
      const json = await res.json();
      setDetailItems(json);
    } catch (err: any) {
      setDetailItems([]);
      setDetailMessage(err.message || 'Erreur');
    } finally {
      setDetailLoading(false);
    }
  };

  const handleKpiDetail = (kpi: ReturnType<typeof kpiCards>[number]) => {
    const params = new URLSearchParams();
    if (dateDebut) params.append('dateDebut', dateDebut);
    if (dateFin) params.append('dateFin', dateFin);

    if (!kpi.detail) {
      fetchDetails(kpi.label, params, 'Les détails ne sont pas disponibles pour cet indicateur.');
      return;
    }

    switch (kpi.label) {
      case 'Consultations':
        fetchDetails('Toutes les consultations', params);
        break;
      case 'Constantes clinique':
        params.append('type', 'constantes');
        fetchDetails('Consultations avec constantes', params);
        break;
      default:
        fetchDetails(kpi.label, params);
    }
  };

  const handleMoisDetail = (mois: string) => {
    const params = new URLSearchParams();
    params.append('mois', mois);
    fetchDetails(`Consultations de ${mois}`, params);
  };

  const handleJourDetail = (jour: string) => {
    const params = new URLSearchParams();
    params.append('jour', jour);
    fetchDetails(`Consultations du ${jour}`, params);
  };
  ...
  const handleMedecinDetail = (medecin: MedecinItem) => {
    ...
    fetchDetails(`Consultations - ${medecin.nom}`, params);
  };
    ...
    <Container fluid className="py-3">
          ...
          <Col key={idx} xs={12} sm={6} md={4} lg={3}>
            <Card className={`border-0 shadow-sm h-100 border-start border-4 border-${kpi.color}`}>
              <Card.Body className="d-flex align-items-center p-3">
                ...
                <button className="btn btn-link btn-sm text-decoration-none p-0 ms-2 flex-shrink-0" title="Voir les détails" onClick={() => handleKpiDetail(kpi)}>
      ...
      <Row className="g-4 mb-4">
        <Col lg={4}>
          <Card className="border-0 shadow-sm h-100 overflow-hidden">
            <Card.Header className="bg-white border-bottom-0 pt-3">
              <h6 className="fw-bold">Répartition des soins par type</h6>
            </Card.Header>
            <Card.Body>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                  <Pie
                    data={data.parTypeSoins.filter(s => s.value > 0)}
                    dataKey="value"
                    nameKey="label"
                    cx="50%"
                    cy="50%"
                    ...
                    label
                  >
                    {data.parTypeSoins.map((entry, index) => (
        ...
        <Col lg={8}>
          <Card className="border-0 shadow-sm h-100 overflow-hidden">
            ...
            <Card.Body>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={data.parMois} onClick={(e: any) => e && e.activeLabel && handleMoisDetail(e.activeLabel)} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="mois" tick={{ fontSize: 12 }} />
                  <YAxis tick={{ fontSize: 12 }} />
                  <ReTooltip />
                  <Legend />
                  <Line type="monotone" dataKey="consultations" name="Consultations" stroke="#0d6efd" strokeWidth={2} />
                  <Line type="monotone" dataKey="constantes" name="Constantes clinique" stroke="#dc3545" strokeWidth={2} />
                  <Line type="monotone" dataKey="soins" name="Soins" stroke="#198754" strokeWidth={2} />
                  <Line type="monotone" dataKey="hospitalisations" name="Hospitalisations" stroke="#ffc107" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </Card.Body>
          </Card>
      ...
      <Row className="g-4 mb-4">
            ...
            <Card.Body>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={filteredMedecins} layout="vertical" margin={{ left: 20, right: 20, top: 10, bottom: 10 }}>
                  ...
                  <ReTooltip />
                  <Legend />
                  <Bar dataKey="consultations" name="Consultations" stackId="a" fill="#0d6efd" onClick={(e: any) => e && e.medecinId && handleMedecinDetail(e as unknown as MedecinItem)} />
                  <Bar dataKey="hospitalisations" name="Hospitalisations" stackId="a" fill="#ffc107" onClick={(e: any) => e && e.medecinId && handleMedecinDetail(e as unknown as MedecinItem)} />
                  <Bar dataKey="soins" name="Soins" stackId="a" fill="#198754" onClick={(e: any) => e && e.medecinId && handleMedecinDetail(e as unknown as MedecinItem)} />
                </BarChart>
              </ResponsiveContainer>
            </Card.Body>
          </Card>
        ...
        <Col lg={5}>
          <Card className="border-0 shadow-sm h-100 overflow-hidden">
            <Card.Header className="bg-white border-bottom-0 pt-3">
              <h6 className="fw-bold">Volume par jour</h6>
            </Card.Header>
            <Card.Body>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={data.parJour} onClick={(e: any) => e && e.activeLabel && handleJourDetail(e.activeLabel)} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 12 }} />
                  <ReTooltip />
                  <Bar dataKey="total" name="Total activité" fill="#0d6efd" />
                </BarChart>
              </ResponsiveContainer>
            </Card.Body>
          </Card>
      ...
      <Modal show={showDetail} onHide={() => setShowDetail(false)} size="xl" fullscreen="lg-down">
        ...
        <Modal.Body>
          {detailLoading ? (
            ...
            <div className="text-center text-muted py-5">{detailMessage}</div>
