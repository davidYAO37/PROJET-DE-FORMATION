...
interface PrintFacturePharmaciePatientProps {
    consultation: any;
}
...
const PrintFacturePharmaciePatient = forwardRef<HTMLDivElement, PrintFacturePharmaciePatientProps>(
    ({ consultation }, ref) => {
        const { entreprise } = useEntreprise();
        const { patient, consultation: consultationData, examens } = consultation || {};
        const [remises, setRemises] = useState<any[]>([]);
        
        // Récupérer les remises de facturation pour la pharmacie
        useEffect(() => {
            const fetchRemises = async () => {
                try {
                    const response = await fetch(`/api/EtatFactureCaisse/reductionFacturePharmacie?codeVisiteur=${consultation.consultation?.Code_consultation}`);
                    ...
                    {remises.length > 0 && (
                        <div style={{ 
                            ...
                            <div style={{ 
                                ...
                                DÉTAIL DES REMISES PHARMACIE
                            </div>
                            <div style={{ padding: '10px', backgroundColor: '#f8f9fa' }}>
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                                    <thead>
                                        <tr style={{ backgroundColor: '#e9ecef' }}>
                                            <th style={{ border: '1px solid #dee2e6', padding: '4px', textAlign: 'left' }}>Médicament</th>
...
PrintFacturePharmaciePatient.displayName = 'PrintFacturePharmaciePatient';

export default PrintFacturePharmaciePatient;