...
export default function ImpressionSoinsExamensPage() {
  ...
  const handlePrint = () => {
    ...
    if (!el) return;
    createPrintWindow(`Analyse & Autres Prestations — ${facture.assurance}`, generatePrintHeader(entreprise), extractContentWithoutHeaderAndFooter(el.innerHTML), generatePrintFooter(entreprise));
  };

  const handlePrintSansEntete = () => {
    const el = document.getElementById('print-content');
    if (!el) return;
    createPrintWindowWithoutHeader('Analyse & Autres Prestations (sans entête)', extractContentWithoutHeaderAndFooter(el.innerHTML));
  };
  ...
  if (parGarant) {
    ...
    lignes.forEach(l => { const k = l.societePatient || '—'; if (!map.has(k)) map.set(k, []); map.get(k)!.push(l); });
  ...
  const renderTableHTML = (rows: Ligne[], tot: Totaux) => {
    const rowsHtml = rows.map((l, i) => {
      ...
      return `<tr style="background:${bg}"><td style="${tdS}">${l.date}</td><td style="${tdS}">${l.beneficiaire}</td><td style="${tdS}">${l.matricule}</td><td style="${tdS}">${l.acte}</td><td style="${tdS};text-align:right">${fmt(l.totalacte)}</td><td style="${tdS};text-align:right">${fmt(l.partAssurance)}</td><td style="${tdS};text-align:right">${fmt(l.partAssure)}</td></tr>`;
    }).join('');
    return `<table style="width:100%;border-collapse:collapse;font-size:11px;margin-bottom:6px"><thead><tr style="background:#4472c4;color:#fff"><th style="${thS}">Date</th><th style="${thS}">Bénéficiaire</th><th style="${thS}">Matricule</th><th style="${thS}">ACTE</th><th style="${thS};text-align:right">Totalacte</th><th style="${thS};text-align:right">Part Assurance</th><th style="${thS};text-align:right">Part assuré</th></tr></thead><tbody>${rowsHtml}<tr style="background:#e0e0e0;font-weight:700"><td colspan="4" style="${tdS};text-align:center;font-weight:700">Total</td><td style="${tdS};text-align:right;font-weight:700">${fmt(tot.totalacte)}</td><td style="${tdS};text-align:right;font-weight:700">${fmt(tot.partAssurance)}</td><td style="${tdS};text-align:right;font-weight:700">${fmt(tot.partAssure)}</td></tr></tbody></table>`;
  };

  const enteteDoc = `<div style="text-align:center;margin-bottom:14px;font-weight:800;font-size:16px;text-decoration:underline">ANALYSE ET AUTRES PRESTATIONS &nbsp; ${facture.assurance}</div><div style="display:flex;justify-content:flex-end;margin-bottom:4px;font-size:11px">Édité Le &nbsp; <strong>${today}</strong></div><div style="margin-bottom:8px;font-size:11px"><strong>ANALYSE ET AUTRES PRESTATIONS</strong>&nbsp;&nbsp; Du &nbsp;<strong>${debutStr}</strong>&nbsp; Au &nbsp;<strong>${finStr}</strong>&nbsp;&nbsp;&nbsp; Ref_Facture &nbsp;<strong>${facture.reference}</strong></div>`;
  ...
  const corpsSansGarant = `<div style="margin-bottom:8px;font-weight:700;text-decoration:underline">DOIT : &nbsp; ${facture.assurance}</div>${renderTableHTML(lignes, totaux)}${makeNetHTML(facture.assurance, totaux.partAssurance)}`;
