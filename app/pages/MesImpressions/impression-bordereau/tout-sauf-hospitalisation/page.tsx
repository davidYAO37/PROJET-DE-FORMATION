...
export default function ImpressionToutSaufHospitalisationPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/comptabilite/impression-bordereau/tout-sauf-hospitalisation?idFactureAssur=${encodeURIComponent(idFactureAssur)}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handlePrint = () => {
    ...
    if (!el) return;
    createPrintWindow('Consultations + Soins + Examens + Pharmacies — ' + facture.assurance, generatePrintHeader(entreprise), extractContentWithoutHeaderAndFooter(el.innerHTML), generatePrintFooter(entreprise));
  };

  const handlePrintSansEntete = () => {
    const el = document.getElementById('print-content');
    if (!el) return;
    createPrintWindowWithoutHeader('Consultations + Soins + Examens + Pharmacies (sans entête)', extractContentWithoutHeaderAndFooter(el.innerHTML));
  };
