...
export default function ImpressionSoinsExamensPharmaciesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/comptabilite/impression-bordereau/soins-examens-pharmacies?idFactureAssur=${encodeURIComponent(idFactureAssur)}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handlePrint = () => {
    ...
    if (!el) return;
    createPrintWindow('Soins + Examens + Pharmacies — ' + facture.assurance, generatePrintHeader(entreprise), extractContentWithoutHeaderAndFooter(el.innerHTML), generatePrintFooter(entreprise));
  };

  const handlePrintSansEntete = () => {
    const el = document.getElementById('print-content');
    if (!el) return;
    createPrintWindowWithoutHeader('Soins + Examens + Pharmacies (sans entête)', extractContentWithoutHeaderAndFooter(el.innerHTML));
  };
