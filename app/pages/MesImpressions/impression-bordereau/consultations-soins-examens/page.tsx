...
export default function ImpressionConsultationsSoinsExamensPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/comptabilite/impression-bordereau/consultations-soins-examens?idFactureAssur=${encodeURIComponent(idFactureAssur)}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handlePrint = () => {
    ...
    if (!el) return;
    createPrintWindow('Liste des Prestations — ' + facture.assurance, generatePrintHeader(entreprise), extractContentWithoutHeaderAndFooter(el.innerHTML), generatePrintFooter(entreprise));
  };

  const handlePrintSansEntete = () => {
    const el = document.getElementById('print-content');
    if (!el) return;
    createPrintWindowWithoutHeader('Liste des Prestations (sans entête)', extractContentWithoutHeaderAndFooter(el.innerHTML));
  };
  ...
  const renderSousTotalHTML = (label: string, tot: Totaux) =>
    `<table style="width:100%;border-collapse:collapse;font-size:11px;margin-bottom:10px">
      <tr style="background:#c6efce;font-weight:700">
        ...
        <td style="${tdS};text-align:right;font-weight:700;background:#c6efce">${fmt(tot.PartAssurance)}</td>
        <td style="${tdS};text-align:right;font-weight:700;background:#c6efce">${fmt(tot.Partassure)}</td>
      </tr>
    </table>`;
  ...
  // SANS GARANT : tout regroupé, DOIT = assurance, 1 seul tableau + sous-total + NET A PAYER
  ...
  // PAR GARANT : un bloc par SOCIETE_PATIENT avec sous-total, puis TOTAL GENERAL + NET A PAYER global
  const corpsParGarant = (() => {
    const blocsGroupes = groupes.map((g, gi) => {
      const gt = {
    ...
    const totalGeneralHTML = `
      <table style="width:100%;border-collapse:collapse;font-size:11px;margin-bottom:4px">
        <tr style="background:#d9d9d9;font-weight:700;font-size:12px">
          <td colspan="4" style="${tdS};text-align:center;font-weight:800">TOTAL GENERAL</td>
          <td style="${tdS};text-align:right;font-weight:800">${fmt(totaux.Totalacte)}</td>
          <td style="${tdS};text-align:right;font-weight:800">${fmt(totaux.PartAssurance)}</td>
          <td style="${tdS};text-align:right;font-weight:800">${fmt(totaux.Partassure)}</td>
        </tr>
      </table>`;

    return `${blocsGroupes}${totalGeneralHTML}${makeNetHTML(facture.assurance, totaux.PartAssurance)}`;
  })();
