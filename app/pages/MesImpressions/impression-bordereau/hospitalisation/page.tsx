...
export default function ImpressionHospitalisationPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/comptabilite/impression-bordereau/hospitalisation?idFactureAssur=${encodeURIComponent(idFactureAssur)}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handlePrint = () => {
    ...
    if (!el) return;
    createPrintWindow('Hospitalisation — ' + facture.assurance, generatePrintHeader(entreprise), extractContentWithoutHeaderAndFooter(el.innerHTML), generatePrintFooter(entreprise));
  };

  const handlePrintSansEntete = () => {
    const el = document.getElementById('print-content');
    if (!el) return;
    createPrintWindowWithoutHeader('Hospitalisation (sans entête)', extractContentWithoutHeaderAndFooter(el.innerHTML));
  };
  ...
  const enteteDoc = `
    ...
    <div style="margin-bottom:10px;font-size:11px">
      <strong>Liste Hospitalisation</strong>&nbsp; Du &nbsp;<strong>${debutStr}</strong>&nbsp; Au &nbsp;<strong>${finStr}</strong>&nbsp;&nbsp;&nbsp; Ref_Facture &nbsp;<strong>${facture.reference}</strong>
    </div>`;
