...
export default function EtatSortiesPage() {
  ...
  const charger = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const params = new URLSearchParams({ entrepriseId, dateDebut, dateFin, typeC: 'Sortie de caisse' });
  ...
  return (
    <div style={{ background: '#f0f4f8', minHeight: '100vh', padding: '8px 10px' }}>
      ...
      {/* HEADER */}
      <div style={{ background: 'linear-gradient(135deg,#b71c1c 0%,#c62828 50%,#e53935 100%)', borderRadius: 8, padding: '8px 16px', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 20px rgba(183,28,28,0.3)' }}>
        <div>
          ...
          <div style={{ color: '#fff', fontSize: '1.05rem', fontWeight: 800, letterSpacing: 1 }}>État des Sorties de Caisse</div>
          <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.72rem', marginTop: 1 }}>{periode}</div>
        </div>
        <i className="bi bi-arrow-up-circle-fill" style={{ fontSize: '1.8rem', color: 'rgba(255,255,255,0.25)' }}></i>
      </div>

      {/* KPIs */}
      <Row className="g-2 mb-2">
        {[
          { label: 'Total Sorties',       value: total,                            bg: 'linear-gradient(135deg,#b71c1c,#ef9a9a)', icon: 'bi-cash-stack' },
          { label: 'Nb Opérations',       value: filtres.length,                   bg: 'linear-gradient(135deg,#1565c0,#42a5f5)', icon: 'bi-list-check', isCount: true },
          { label: "Types d'opérations",  value: Object.keys(parOperation).length, bg: 'linear-gradient(135deg,#880e4f,#f48fb1)', icon: 'bi-bar-chart-fill', isCount: true },
        ].map((kpi, ki) => (
      ...
      {Object.keys(parOperation).length > 0 && (
        <Row className="g-2 mb-2">
          {Object.entries(parOperation).map(([op, val], i) => (
            <Col key={i} xs={6} md={3}>
              <div style={{ background: '#fff', borderRadius: 8, padding: '6px 10px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', borderLeft: '3px solid #b71c1c' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#546e7a', textTransform: 'uppercase' }}>{op}</span>
                  <Badge bg="danger" style={{ fontSize: '0.58rem' }}>{val.count}</Badge>
                </div>
                <div style={{ fontWeight: 800, fontSize: '0.85rem', color: '#b71c1c' }}>{fmt(val.total)} F</div>
              </div>
            </Col>
          ))}
        </Row>
        ...
        <Card.Body style={{ padding: '8px 14px' }}>
          <Row className="g-2 align-items-end">
            ...
            <Col xs="auto" className="ms-auto d-flex gap-2">
              <Button onClick={charger} disabled={loading} style={{ background: 'linear-gradient(135deg,#b71c1c,#c62828)', border: 'none', fontWeight: 700, fontSize: '0.78rem', padding: '5px 14px', borderRadius: 6 }}>
      ...
      <Card style={{ borderRadius: 8, border: 'none', boxShadow: '0 1px 6px rgba(0,0,0,0.07)' }}>
        <div style={{ background: 'linear-gradient(90deg,#b71c1c,#c62828)', color: '#fff', padding: '7px 14px', borderRadius: '8px 8px 0 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontWeight: 700, fontSize: '0.78rem', letterSpacing: 1 }}><i className="bi bi-table me-2"></i>SORTIES DE CAISSE — {periode}</span>
              ...
              <thead>
                <tr style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                  {['Date','Opération','Motif','Service','Personne','Saisi par','Montant'].map((h, hi) => (
                    <th key={hi} style={{ background: hi === 6 ? '#ffcdd2' : '#cfd8dc', color: hi === 6 ? '#b71c1c' : '#37474f', padding: '6px 8px', fontWeight: 700, whiteSpace: 'nowrap', textAlign: hi === 6 ? 'right' : 'left', borderRight: '1px solid #b0bec5' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={7} style={{ textAlign: 'center', padding: '50px', color: '#90a4ae' }}>
                    <i className="bi bi-inbox" style={{ fontSize: '2.5rem', display: 'block', marginBottom: 8 }}></i>Aucune sortie pour cette période
                  </td></tr>
                ) : filtres.map((d, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#fff5f5', borderLeft: '3px solid #b71c1c' }}>
                    ...
                    <td style={{ padding: '3px 8px', textAlign: 'right', fontWeight: 'bold', color: '#b71c1c' }}>-{fmt(d.MOntantC || 0)}</td>
                  </tr>
                ))}
              </tbody>
            </Table>
        ...
        <div style={{ background: '#eceff1', borderRadius: '0 0 8px 8px', padding: '6px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '2px solid #cfd8dc' }}>
          ...
          <div style={{ textAlign: 'right' }}><div style={{ fontSize: '0.62rem', fontWeight: 700, color: '#b71c1c', textTransform: 'uppercase' }}>Total Sorties</div><div style={{ fontSize: '1rem', fontWeight: 800, color: '#b71c1c' }}>-{fmt(total)} F</div></div>
        </div>
      </Card>
    </div>
  );
