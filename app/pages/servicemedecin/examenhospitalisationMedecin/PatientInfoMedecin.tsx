...
type Props = {
    ...
    codePrestation?: string; // Ajout du codePrestation en prop
};

export default function PatientInfoMedecin({ formData, setFormData, onCodePrestationChange, codePrestation }: Props) {
    const [CodePrestation, setCodePrestation] = useState(codePrestation || "");
    const [patientNom, setPatientNom] = useState("");
    const [infoMessage, setInfoMessage] = useState<string | null>(null);
    const [errorMessage, setErrorMessage] = useState<string | null>(null);

    // Synchroniser le CodePrestation quand la prop change
    useEffect(() => {
        if (codePrestation && codePrestation !== CodePrestation) {
            setCodePrestation(codePrestation);
            if (onCodePrestationChange) {
                onCodePrestationChange(codePrestation);
            }
        }
    }, [codePrestation, CodePrestation, onCodePrestationChange]);
