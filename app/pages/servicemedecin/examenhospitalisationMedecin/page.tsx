...
import PatientInfoMedecin from "./PatientInfoMedecin";
import AssuranceInfoMedecin from "./AssuranceInfoMedecin";
import ActesTableMedecin from "./ActesTableMedecin";
import CliniqueInfoMedecin from "./CliniqueInfoMedecin";
import ActionsButtonsMedecin from "./ActionsButtonsMedecin";
import PaiementInfoMedecin from "./PaiementInfoMedecin";

export default function HospitalisationPageMedecin() {
    ...
    useEffect(() => {
        ...
        if (codePrestationFromConsultation) {
            ...
            // Ne pas nettoyer immédiatement pour permettre l'affichage
            setTimeout(() => {
                localStorage.removeItem('codePrestationConsultation');
            }, 1000);
        }
    }, []);

    // useEffect pour écouter les changements de codePrestation dans le localStorage
    useEffect(() => {
        ...
        // Vérifier toutes les 300ms pour détecter les changements (plus fréquent)
        const interval = setInterval(checkCodePrestation, 300);

        // Ajouter l'écouteur d'événements personnalisés
        window.addEventListener('codePrestationChanged', handleCodePrestationChange);

        return () => {
            clearInterval(interval);
            window.removeEventListener('codePrestationChanged', handleCodePrestationChange);
        };
    }, [CodePrestation]);
