...
export async function GET(req: NextRequest, { params }: { params: Promise<{ name: string }> }) {
  try {
    ...
    if (!medecin) {
      // Pour le débogage : lister tous les médecins disponibles
      const allMedecins = await Medecin.find({}).select('nom prenoms').limit(10);
      
      return NextResponse.json(
        { 
          error: 'Médecin non trouvé',
          searchedFor: fullName,
          availableMedecins: allMedecins.map(m => `${m.nom} ${m.prenoms}`)
        },
        { status: 404 }
      );
    }
