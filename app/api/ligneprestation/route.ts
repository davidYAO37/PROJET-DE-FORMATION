...
export async function GET(req: NextRequest) {
    try {
        ...
        // Sinon, récupérer les lignes par CodePrestation, idHospitalisation ou patientId
        if (!CodePrestation && !idHospitalisation && !patientId) {
            return NextResponse.json(
                { error: "Paramètre manquant", message: "Le code de prestation, l'idHospitalisation, le patientId ou l'ID est requis" },
                { status: 400 }
            );
        }
